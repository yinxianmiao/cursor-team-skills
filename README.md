...
| 同步 Rules + Prompts | Cursor 打开本仓库 → 终端 `npm run sync -- --preset …` | 每个业务仓首次一次；团队改了 `rules/` / `prompts/` 后再跑 |

Skill 管工作流（装到 `~/.agents/skills/`）；细则在本仓 `rules/`；提问模板在本仓 `prompts/`，再 sync 到业务仓 `docs/`。业务仓只手写 `*.local.mdc` 与 `docs/project-map.md`。

> **重要：** 用 Cursor **打开本仓库**，在本仓库终端里跑 `skills:install` / `skills:verify` / `sync`。  
> 打开业务项目再跑会报 `Missing script`——业务仓没有这些脚本。
...
## 2. 同步 Rules + Prompts 到业务仓

用 Cursor **打开本仓库（cursor-team-skills）**，在本仓库的终端里执行（不要打开业务仓再跑）。**一条命令**同时写入 `.cursor/rules/` 与 `docs/prompts*`：
...
```bash
npm run sync -- --preset uni-app --target /Users/你的用户名/Desktop/云畔/geological-museum-system/geological-museum-system-wx
```
...
```text
业务仓/.cursor/rules/
├── 00-encoding.mdc               # sync（shared），勿手改
├── vue3/base.mdc、api-js.mdc     # sync，勿手改
├── uni-app/*.mdc                 # uni-app preset；勿手改
├── admin/*.mdc                   # admin preset；勿手改
├── */project.local.mdc           # 本仓手写，sync 不覆盖
└── SYNC.md

业务仓/docs/
├── prompts.md                 # sync，勿手改
├── prompts/*.md               # sync，勿手改（admin 含 conventions.md）
├── PROMPTS-SYNC.md            # sync 说明
└── project-map.md             # 本仓手写，sync 不覆盖
```

业务仓首次请自备：`uni-app/project.local.mdc` 或 `admin/project.local.mdc`（可选，承载本项目差异）；`docs/project-map.md` 用 Skill 内模板自建。

仅同步其一（可选）：`npm run rules:sync` / `npm run prompts:sync`（参数相同）。说明见 [prompts/README.md](./prompts/README.md)。
...
若本次 pull 改了 `rules/` 或 `prompts/`，再对各业务仓跑一遍 `npm run sync -- --preset … --target …`。勿在业务仓手改非 local 的 `.mdc` 与 sync 下来的 `docs/prompts*`。
